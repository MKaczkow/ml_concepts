...
class GenerateEmbeddingsTrain(Callback):
    ...
    def on_train_epoch_end(self, trainer, pl_module):
        ...
        torch.save(embeddings, f"{self.save_dir}/embeddings_train_{trainer.current_epoch}.pth")

        # save tsne embeddings
        tsne = TSNE(n_components=2)
        h_embedded = tsne.fit_transform(embeddings.cpu().detach().numpy())
        torch.save(h_embedded, f"{self.save_dir}/embeddings_train_tsne_{trainer.current_epoch}.pth")

        # plot tsne embeddings
        plt.scatter(h_embedded[:, 0], h_embedded[:, 1])
        plt.savefig(f"{self.save_dir}/embeddings_train_tsne_plot_{trainer.current_epoch}.png")
        # plt.show()